...
## Turborepo 레포지토리 구조 잡기

Turborepo는 워크스페이스(Workspace) 개념을 기반으로 동작합니다. 모노레포 구조 설계와 패키지 간 관계 정의 방법을 정리합니다.
...
Turborepo 레포지토리 구조의 핵심은 워크스페이스 기반 구성, `package.json`을 통한 명시적 의존성 선언, exports를 활용한 진입점 정의입니다. 설정 파일은 패키지로 관리하고, 패키지 간 접근은 상대 경로 대신 패키지 이름을 사용합니다.

다음 글에서는 `turbo.json`을 통한 태스크 설정과 파이프라인 구성을 다룹니다.
